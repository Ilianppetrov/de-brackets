...
(function ($) {
  ...
  var TeamBlock = /** @class */ (function () {
    function TeamBlock(
      source, // Where base of the information propagated from
      nameOrGetter,
      order,
      seed,
      score
  ...
  var loserAlignment = function (teamCon, match) {
    return function () {
      return teamCon.css(
        "top",
        match.el.height() / 2 - teamCon.height() / 2 + "px"
      );
    };
  };
  ...
  var Round = /** @class */ (function () {
    ...
    Round.prototype.addMatch = function (teamCb, renderCb) {
      ...
      var match = this.mkMatch(
        this,
        matchResult,
        matchIdx,
        this.roundResults.map(function (r) {
  ...
  function mkConnector(roundMargin, connector, teamCon, align) {
    ...
    if (shift >= 0) {
      src.css("top", connector.containerHeight - shift - 1 + "px");
    } else {
      ...
      return brackets.map(function (rounds) {
        return rounds.map(function (matches) {
          var matchData = [
            ...
            matches?.teamANumber?.val,
            matches?.teamBNumber?.val,
          ];
          ...
          function editor() {
            ...
            input.keydown(function (e) {
              ...
              if (key === 9 || key === 13 || key === 27) {
                ...
                var next = topCon.find(
                  "div.score[data-resultid=result-" + (rId_1 + 1) + "]"
                );
                if (next) {
                  next.click();
                }
  ...
  var Match = /** @class */ (function () {
    ...
    Match.prototype.connect = function (cb) {
      ...
      const containerHeight = this.teamCon.height();
      var connectorOffset = this.teamCon.children(".team").first().height() / 2;
      console.log(connectorOffset);
                ...
                return order.map(
                  {
                    ...
                    shift:
                      connectorOffset * (_this.opts.centerConnectors ? 2 : 1),
                    containerHeight,
                  },
                  {
                    containerHeight,
                    ...
                    shift:
                      connectorOffset * (_this.opts.centerConnectors ? 2 : 3),
                  }
                );
              })
              .orElse({
    ...
    Match.prototype.render = function () {
      ...
      var isReady =
        !this.match.a.name.isEmpty() && !this.match.b.name.isEmpty();
      this.teamCon.append(timepicker(this.setTime));
    ...
    Match.prototype.results = function () {
      ...
      return new ResultObject(
        ...
        this.match.b.score,
        this.matchUserData,
        this.match.a.nameOrGetter,
        this.match.b.nameOrGetter
      );
    };
    return Match;
  })();
  ...
  var JqueryBracket = function (opts) {
    ...
    var topCon = $('<div class="jQBracket ' + opts.dir + '"></div>').appendTo(
      opts.el.empty()
    );
    function resizeContainer() {
      var roundCount = countRounds(
        data.teams.length,
        isSingleElimination,
